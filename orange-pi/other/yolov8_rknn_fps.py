...
if __name__ == '__main__':
    ...
    # img_src = cv2.imread('sample_1.jpg')
    vid = cv2.VideoCapture(0)

    while True:
        new_frame_time = time.time()
        ret, img_src = vid.read()
        if img_src is None:
            print('[failed to read]')
            exit(-1)

        pad_color = (0,0,0)
        img = letter_box(img_src.copy(), new_shape=(640, 640), pad_color=(0,0,0))
        # img = cv2.resize(img_src, (640, 512), interpolation=cv2.INTER_LINEAR)
        ...
        if boxes is not None:
            ...
            # print([CLASSES[cl] for cl in classes])
            # for i, element in enumerate(classes):
            #     object = CLASSES[element]
            #     if object == 'person':
            #         print('person detected')
            draw(img_p, boxes, scores, classes)
        
        # cv2.imwrite('sample_result.jpg', img_p)
        fps = 1/(new_frame_time-prev_frame_time)
        prev_frame_time = new_frame_time

        fps = int(fps)
        fps_string = str(fps)
        cv2.putText(img_p, fps_string, (7, 70), cv2.FONT_HERSHEY_SIMPLEX, 3, (100, 255, 0), 3, cv2.LINE_AA)

        cv2.imshow('frame', img_p)
